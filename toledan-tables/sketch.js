function Planet(r, start, speed, name, pcolor){
  ...
  Planet.ofTableRName = function(mensium, collectisRadix, r, name, pcolor){
    ...
    M_0 = collectisRadix[0] * 30 + collectisRadix[1] + collectisRadix[2] / 60 + collectisRadix[3] / 3600;
  ...
  function setup() {
    ...
    sun = Planet.ofTableRName([0, 0, 2, 0], [0, 0, 59, 8], 93 * SCALE, "Sun", color(255,255,0));
    saturn = Planet.ofTableRName([0, 0, 2, 0], [3, 25, 51, 15], 746 * SCALE, "Saturn", color(200,160,120)); // millions of miles of distance from earth
    jupiter = Planet.ofTableRName([0, 0, 4, 59], [11, 1, 39, 37], 390 * SCALE, "Jupiter", color(255,120,0));
    mars = Planet.ofTableRName([0,0, 31, 26], [7, 1, 24, 59], 140 * SCALE, "Mars", color(255,60,0));
    venus = Planet.ofTableRName([0, 0, 37, 0], [1, 15, 28, 37], 25 * SCALE, "Venus", color(230,200,190));
    mercury = Planet.ofTableRName([0, 3, 6, 24], [2, 13, 46, 18], 57 * SCALE, "Mercury", color(120,120,120));
    ...
    for(var i=0;i<100;i++){
      starPositions.push({x: random(-width/2, width/2), y: random(-height/2, height/2)});
    }
